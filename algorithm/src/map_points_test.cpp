...
TEST(MapPointsTest, OrdersPointsAsExpected) {
	...
	EXPECT_EQ(mp->errands->at(0)->lat, 51.49841775633878);
	EXPECT_EQ(mp->errands->at(0)->lng, -0.17533063886503442);

	EXPECT_EQ(mp->errands->at(1)->lat, 51.497629616981217);
	EXPECT_EQ(mp->errands->at(1)->lng, -0.17346382139066919);

	EXPECT_EQ(mp->errands->at(2)->lat, 51.497843351035147);
	EXPECT_EQ(mp->errands->at(2)->lng, -0.1702022552285598);

	EXPECT_EQ(mp->errands->at(3)->lat, 51.49644070301275);
	EXPECT_EQ(mp->errands->at(3)->lng, -0.17026662824491724);
